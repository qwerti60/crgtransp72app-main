...
if ($rollNum === '1'||$rollNum === '4') {
    ...
    // Если фильтр flag не передан — показываем всех пользователей,
    // чтобы новые записи (часто с flag=0) не пропадали из списка.
    $sql = "SELECT * FROM users WHERE rollNum = '$rollNum' AND statNum = '$statNum'";
    $link = null;
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        foreach ($row as $key => $value) {
            if (in_array($key, ['rollNum', 'statNum', 'flag', 'password'], true)) {
                continue;
            }
            // BLOB фото: не прогонять через htmlspecialchars — портит байты и может ломать строку.
            if ($key === 'fotouser') {
                $validValue = $value !== null && is_string($value) && strlen($value) > 0;
                if ($validValue) {
                    $eligibleColumns[$key] = true;
                }
                $rowData[$key] = $validValue ? $value : '';
                continue;
            }
            ...
            $rowData[$key] = $validValue ? htmlspecialchars((string) $value, ENT_QUOTES, 'UTF-8') : '';
        }
        $dataRows[] = $rowData;
    }
