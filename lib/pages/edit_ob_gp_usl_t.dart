...
class edit_ob_gp_usl_t extends StatefulWidget {
  final int id;
  //const edit_ob_gr({super.key, required this.id});
  const edit_ob_gp_usl_t(
      {super.key, required this.id}); // const edit_ob_gp_usl_t({super.key});
...
class _add_ob_gpForm extends State<edit_ob_gp_usl_t> {
...
// Можно указать типы файлов, добавив параметры в pickImage
  ...
  void uploadData() async {
    var uri = Uri.parse('${Config.apiBase}/edit_st_info.php');
...
// Форматируем дату для отображения
  ...
  DateTime? _tryParseDate(String? value) {
    // всё, что считаем «пустой датой»
    const _emptyVariants = {
      '',
      '0000-00-00',
      '0000-00-00 00:00:00',
    };
    if (value == null || _emptyVariants.contains(value)) return null;

    // пробуем сначала ваш формат
    try {
      return _dateFormat.parse(value);
    } catch (_) {
      // затем ISO-8601 (DateTime.tryParse вернёт null, а не бросит ошибку)
  ...
  Future<List<Map<String, dynamic>>> fetchAds() async {
    ...
    final uri = Uri.parse(Config.baseUrl).replace(
      path: '${Config.apiPrefix}/edit_ob_t.php',
      queryParameters: {
    ...
    try {
      ...
      if (ads.isNotEmpty) {
        ...
        setState(() {
          ...
          _aboutController.text = ad['about'] ?? '';
          _startDate = _tryParseDate(ad['startdate']);

          _endDate = _tryParseDate(ad['enddate']);

          _endDatez = _tryParseDate(ad['enddatez']);

          selectedDatez = _endDatez!;
          _cenakmController.text = ad['cena'] ?? '';
          if (_startDate == null && _endDate != null)
            _selectedOption = 'Не позднее чем';
          if (_startDate != null && _endDate != null)
            _selectedOption = 'В период с... по...';
          for (var i = 0; i < 4; i++) {
            // 0,1,2,3
...
// Если хотя бы одно поле пустое, показываем осведомительное сообщение
                    ...
                    child: const Text('Сохранит заказ')),
              ),
            ),
          ],
        ),
