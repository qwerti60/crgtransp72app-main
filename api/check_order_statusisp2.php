...
try {
    ...
    // Проверяем наличие записи в таблице ordersglobal
    $stmtCheckOrdersGlobal = $pdo->prepare("SELECT * FROM ordersglobal WHERE order_id = :order_id");
    ...
    if (!$recordExistsInOrdersGlobal) {
        // Проверяем таблицу offer_data на наличие свободных заказов
        $stmtCheckOfferData = $pdo->prepare("SELECT * FROM offer_data WHERE iduserp = :iduserp AND iduser = :iduser AND status = 0 LIMIT 1");
        ...
        if (!empty($offerRow)) {
            ...
            $stmtInsertOrdersGlobal = $pdo->prepare("INSERT INTO ordersglobal (user_id, order_id, user_idok, start_time, status, idoffer) VALUES (:user_id, :order_id, :user_idok, :start_time, 'выполняется', :idoffer)");
            ...
            // Запись со статусом 0 не найдена
            echo json_encode([
                'message' => 'Выполняется другим исполнителем'
            ]);
        }
    } else {
