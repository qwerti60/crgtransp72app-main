...
if (isset($_SESSION['admin_web_token']) && is_string($_SESSION['admin_web_token']) && $_SESSION['admin_web_token'] !== '') {
    ...
    if (tp_admin_authorized(tp_pdo())) {
        header('Location: cities.php', true, 302);
        exit;
    }
}
...
<head>
    ...
    <style>
        ...
        input[type=text], input[type=password], input[type=email] { width: 100%; box-sizing: border-box; padding: 0.5rem; margin-top: 0.25rem; }
        button { margin-top: 1rem; padding: 0.5rem 1rem; cursor: pointer; }
        .err { color: #b00020; margin-top: 1rem; }
        .info { color: #15803d; margin-top: 1rem; }
...
<body>
    ...
    <p class="meta">Укажите логин. На e-mail из учётной записи (<code>admin_accounts.email</code>) придёт 6-значный код. E-mail задаётся в разделе «Настройки» после входа.</p>
    <?php if ($error !== '') { ?>
    ...
    <form method="post" action="login_reset.php" autocomplete="off">
        ...
        <label for="np">Новый пароль (не короче <?= $minLen ?> символов)</label>
        <input id="np" name="new_password" type="password" required minlength="<?= $minLen ?>" maxlength="256" autocomplete="new-password">
        <label for="np2">Повторите пароль</label>
        <input id="np2" name="new_password2" type="password" required minlength="<?= $minLen ?>" maxlength="256" autocomplete="new-password">
