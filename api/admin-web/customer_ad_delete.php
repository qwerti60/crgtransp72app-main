...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['delete_ad'])) {
    header('Location: customer_ads.php', true, 302);
    exit;
}
...
$type = crg_admin_customer_type_from_request() ?? 'orders';
...
if ($res === true) {
    header('Location: customer_ads.php?type=' . rawurlencode($type) . '&deleted=1', true, 303);
    exit;
}
