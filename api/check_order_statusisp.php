...
try {
    ...
    $stmtCheckOrderExecuting = $pdo->prepare("SELECT * FROM ordersglobal WHERE user_id = :userIdok AND status='выполняется'");
    ...
    $stmtCheckOrderCompleted = $pdo->prepare("SELECT * FROM ordersglobal WHERE user_id = :userIdok AND status='выполнен' ORDER BY id DESC"); // Добавляем сортировку по убыванию ID
    ...
    while ($completedOrder = $stmtCheckOrderCompleted->fetch(PDO::FETCH_ASSOC)) {
        ...
        $stmtCheckReview->bindValue(':userId', $completedOrder['user_id'], PDO::PARAM_INT); // user_id из ordersglobal
    //    $stmtCheckReview->bindValue(':targetUserId', $completedOrder['order_id'], PDO::PARAM_INT); // user_idok из ordersglobal
